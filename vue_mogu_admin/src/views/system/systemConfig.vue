    ...
    <el-tabs type="border-card" @tab-click="handleClick" v-model="activeName">
      <el-tab-pane name="one" v-permission="'/systemConfig/getSystemConfig'">
        ...
        <el-form style="margin-left: 20px;" label-position="left"   label-width="140px" >
          ...
          <el-form-item label="选用消息队列">
            <el-radio v-for="item in mqTypeDictList" :key="item.uid" v-model="form.mqType" :label="item.dictValue" border size="medium">{{item.dictLabel}}</el-radio>
          </el-form-item>
...
export default {
  ...
  methods: {
    ...
    getDictList: function () {
      ...
      var dictTypeList =  ['sys_search_model', 'sys_yes_no', 'sys_mq_type', 'sys_picture_priority', 'sys_storage_region', 'sys_normal_disable', 'sys_editor_modal']
      ...
      getListByDictTypeList(dictTypeList).then(response => {
        if (response.code == this.$ECode.SUCCESS) {
          ...
          this.mqTypeDictList = dictMap.sys_mq_type.list
