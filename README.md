...
<p align=center>
  <a href="http://www.moguit.cn">
    <img src="./doc/images/gitee/favicon2.ico" alt="白的博客" style="width:200px;height:200px">
  </a>
</p>
<p align=center>
   白的博客，一个基于微服务架构的前后端分离博客系统
</p>
<p align="center">
...
白的博客( **MoguBlog** )，一个**基于微服务架构的前后端分离博客系统**。**Web** 端使用 **Vue** + **ElementUi** , 移动端使用 **uniapp** 和 **ColorUI**。后端使用 **SpringCloud** + **SpringBoot** + **Mybatis-plus**进行开发，使用 **Jwt** + **SpringSecurity** 做登录验证和权限校验，使用 **ElasticSearch** 和 **Solr** 作为全文检索服务，使用 **Github Actions**完成博客的持续集成，使用 **ElasticStack** 收集博客日志，文件支持**上传本地**、**七牛云** 和 **Minio**.

- 白的博客大部分功能是我个人进行开发的，因能力有限，其中很多技术都是一边学习一边使用的，可以说白的博客也是一个我用来熟悉技术的项目，所以很多地方可能考虑不周，故有能改正的地方，还请各位老哥能够指出~
...
- [白的博客](http://moguit.cn/#/)中的一些文章，很多都来自我平时的学习笔记，目前托管在另一个仓库：[LearningNotes](https://gitee.com/moxi159753/LearningNotes)，里面涵盖了 **JVM**、**JUC**、**Java**，**Spring**，**SpringCloud**，计算机网络，操作系统，数据结构，**Vue**、**Linux** 等 ，感兴趣的小伙伴可以跳转该仓库 [Star支持](https://gitee.com/moxi159753/LearningNotes) 一下

## 运行配置

白的博客使用了一些监控的 **SpringCloud** 组件，但是并不一定都需要部署，必须启动的服务包含
...
首先特别感谢群里小伙伴 **@泪梦红尘** 对白的博客提供服务器支持。所以就刚好以此用来做白的博客的演示站点，同时我也在搭建的时候，顺便录制了一套视频：[20分钟部署一个微服务架构的博客系统](https://www.bilibili.com/video/BV13y4y1V7Us) ，感兴趣的小伙伴可以参考视频一起完成部署~
...
>
> 【小程序】白的博客的移动端版本，可以扫码体验。参考 [白的博客小程序部署](http://www.moguit.cn/#/info?blogOid=605) 文档，以及Bilibili上 [20分钟部署博客小程序视频](https://www.bilibili.com/video/BV1sQ4y117nN) 完成小程序的搭建。
...
[**mogu_blog_go**](https://gitee.com/moxi159753/mogu_blog_go) 是根据**白的博客**改编的一个 **Golang** 简化版分支，实现了白的博客后台原有的大部分**基础功能**，同时前端代码和数据库基本没有变化，因此可以直接使用白的博客原有的前端页面。

[**mogu_blog_go**](https://gitee.com/moxi159753/mogu_blog_go) 目前采用的技术有：**beego**、**gorm**、**mysql**、**redis** 和 **nginx**。相比于白的博客微服务版，**Golang**版蘑菇能够做到 **1C2G** 的服务器轻松运行，再也不怕网站第二天起来就宕机了~。

参考[白的博客Golang版安装指南](http://moguit.cn/#/info?blogOid=593)，快速在 **Windows** 平台搭建白的博客Go版开发环境
...
- **白的博客监控页面**：用户：user，密码：password123
...
- 引入**Github Actions** 工作流，完成白的博客的持续集成、持续部署。
...
- 采用[uniapp](https://uniapp.dcloud.io/) 和[ColorUi](https://github.com/weilanwl/ColorUI) 完成白的博客的移动端门户页面搭建
...
- 采用 **ElasticStack**【**ElasticSearch** + **Beats** + **Kibana** + **Logstash**】[搭建白的博客日志收集](http://moguit.cn/#/info?blogOid=436)
...
- mogu_base: 是一些Base基类
- doc: 是白的博客的一些文档和数据库文件
- vue_mogu_admin：VUE的后台管理页面
- vue_mogu_web：VUE的门户网站
- uniapp_mogu_web：基于uniapp 和 colorUi 的白的博客移动端门户页面（Nacos分支）
...
>  白的博客系统架构图，使用 [Processon](https://www.processon.com/i/5e380df1e4b05b335ffa81e9) 在线绘制
...
参考 [如何使用一条命令完成白的博客部署](http://www.moguit.cn/#/info?blogOid=597)， 在云服务器上执行下面命令，完成一键部署白的博客
...
### [推荐] Windows+VMware一键搭建白的博客本地开发环境

参考 [Windows+VMware一键搭建白的博客本地开发环境](http://www.moguit.cn/#/info?blogOid=583)，快速在 **Windows** 平台搭建白的博客开发环境，将中间件安装在 **VMware** 中。

### Windows环境下搭建白的博客

参考 [window环境下配置白的博客环境](http://www.moguit.cn/#/info?blogUid=082ca226cf2e4103b0ffa6e6c13d7b14)，能够在 **Windows**下搭建白的博客的开发环境，同时将**所有组件**安装在**Windows** 环境中

### [推荐] DockerCompose一键部署白的博客(Nacos版)

参考 [DockerCompose一键部署白的博客(Nacos版)](http://www.moguit.cn/#/info?blogOid=565) ，在 **Linux** 服务器中，一键完成白的博客项目的部署

### Docker搭建白的博客

参考 [使用Docker快速搭建白的博客(Eureka分支)](http://www.moguit.cn/#/info?blogUid=ab8377106a0d4b9f8d66131e4312c69e)，快速在Linux服务器中搭建好博客运行环境，通过发布 **Jar** 包的方式完成部署

参考 [使用Docker快速搭建白的博客(Nacos分支)](http://moguit.cn/#/info?blogUid=8100dcb585fff77e3fa25eed50e3708e)，快速在Linux服务器中搭建好博客运行环境，通过发布 **Jar** 包的方式完成部署

### 白的博客部署到云服务器

【**推荐**】参考[IDEA发布白的博客Docker镜像到云服务器(适用于DockerCompose部署)](http://moguit.cn/#/info?blogOid=588)，在你修改Nacos分支博客源码后，将重新制作镜像部署到云服务器，然后通过DockerCompose进行启动。

参考[白的博客如何部署到阿里云服务器(Eureka分支)](http://moguit.cn/#/info?blogOid=238)，在你修改Eureka分支博客源码后将项目打包部署到云服务器

参考[白的博客如何部署到阿里云服务器(Nacos分支)](http://moguit.cn/#/info?blogOid=405)，在你修改Nacos分支博客源码后将项目打包部署到云服务器

### 使用Github Actions完成白的博客持续集成

参考 [使用Github Action完成白的博客持续集成](http://moguit.cn/#/info?blogOid=13) ，在你提交一个 **push** 请求后，通过 **Github Actions** 能够完成白的博客自动化 编译、打包、部署等操作。

### 切换搜索模式

参考[白的博客切换搜索模式](http://moguit.cn/#/info?blogOid=119)，完成白的博客的搜索引擎切换，目前支持 **Solr**、**ElasticSearch**、**SQL** 的方式

### 配置七牛云对象存储

参考[白的博客切换七牛云存储](http://moguit.cn/#/info?blogOid=202)，配置文件的七牛云对象存储，及本地文件存储

### 使用Zipkin搭建白的博客链路追踪

参考[使用Zipkin搭建白的博客链路追踪](http://moguit.cn/#/info?blogUid=35bd93cabc08611c7f74ce4564753ef9)，通过聚合各业务系统调用延迟数据，达到链路调用监控跟踪，快速定位其中延迟高的服务

### 使用Nacos搭建白的博客服务注册和配置中心

参考[白的博客Nacos部署指南](http://www.moguit.cn/#/info?blogUid=8dc52bd61e36fa56cfc7699815375572)，搭建白的博客的服务注册和配置中心

### 使用Sentinel搭建白的博客流量控制

参考[白的博客Sentinel安装指南](http://www.moguit.cn/#/info?blogUid=7135efc7f536769efd0d0483c687ba07)，**Sentinel** 被称为分布式系统的流量防卫兵，相当于Hystrix

### 白的博客QQ小程序发布

参考[白的博客QQ小程序发布指南](http://www.moguit.cn/#/info?blogUid=80d3eae77c16cea10e119b9f1a1da4c8)，完成白的博客 **uniapp** 移动端的启动和发布

### 【推荐】白的博客微信小程序发布

参考[白的博客微信小程序部署指南](http://www.moguit.cn/#/info?blogOid=605)，完成白的博客 **uniapp** 移动端的启动和发布

### 白的博客扩展新的功能和页面

参考[白的博客如何扩展新的功能和页面](http://moguit.cn/#/info?blogUid=4eb7694c58cf8205885b1e8565b1a94e)，在白的博客现有架构基础上，开发自己需要的页面。

###  使用ELK搭建白的博客日志收集

参考[搭建白的博客日志收集](https://gitee.com/moxi159753/LearningNotes/tree/master/ElasticStack/6_%E4%BD%BF%E7%94%A8ELK%E6%90%AD%E5%BB%BA%E8%98%91%E8%8F%87%E5%8D%9A%E5%AE%A2%E6%97%A5%E5%BF%97%E6%94%B6%E9%9B%86)，使用 **Docker** 快速搭建 **ELK** 环境用于白的博客日志收集
...
**白的博客**起初参考了很多**开源项目**的**解决方案**，**开源不易，感谢分享**
...
为了方便小伙伴们沟通交流，我创建了QQ群 (**加群备注**：`白的博客`)  和 **微信群**（备注：**加群**），目前项目还存在很多不足之处，欢迎各位老哥进群进行技术交流，为了防止广告进入，希望加群的时候能添加备注，谢谢~

|                   微信群【备注：加群】                   |          ~~QQ群（备注：`白的博客`）【群满】~~          | QQ群（备注：`白的博客`）【推荐】                       |
| :------------------------------------------------------: | :----------------------------------------------------: | ------------------------------------------------------ |
...
- [x] 集成Github Actions，完成白的博客持续集成服务
- [x] 门户网站增加登录页面
- [x] 支持第三方登录
...
- [x] 增加白的博客小程序项目 uniapp_mogu_web，基于[ColorUI](https://github.com/weilanwl/ColorUI) 和 [Uniapp](https://uniapp.dcloud.io/)
- [x] 富文本编辑器和Markdown编辑器任意切换
- [x] 使用ELK搭建[白的博客日志收集功能](http://www.moguit.cn/#/info?blogUid=fd9ab58588d93ef792ec72a359a09f6c)
...
- [ ] 增加博客迁移功能，让其它平台的博客，如：CSDN、博客园，WordPress能够同步到白的博客中

## 贡献代码

开源项目离不开大家的支持，如果您有好的想法，遇到一些 **BUG** 并修复了，以及 [白的博客文档](https://gitee.com/moxi159753/mogu_blog_doc) 上有错误的地方订正过来了，欢迎小伙伴们提交 **Pull Request** 参与开源贡献
